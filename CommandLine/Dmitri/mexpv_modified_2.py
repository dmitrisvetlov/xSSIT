...
def mexpv_modified_2(
        t: int, A: csr_array, v: np.ndarray,
        tol: float = 1.0e-7,
        m: int = None,
    ...
    if SINKS is None:
        SINKS = np.array([], dtype = np.int64)
        
    if fspErrorCondition is None:
        fspErrorCondition = FSPErrorCondition()
        
    # Scipy does not allow the calculation of infinite norms of sparse 
    # matrices, so we must convert to a dense representation to calculate it:
    Adense = A.todense()
    Anorm = norm(Adense, ord = np.inf)
    ...
    while ((tNow < t_out) and (i_prt < Time_array.size)):
        ...
        for j in range(m):
            ...
            for i in range(j):
                H[i,j] = p.T @ V[:, i] #8.832
                # Since H[i, j] is a single element, we must use the scalar
                # multiplication operand (*) rather than the matrix one (@):
                p = p - H[i, j] * V[:, i]
        ...
        if k1 != 0:
            H[m + 1, m] = 1
            AVnorm = norm(A @ V[:, m], ord = 2) # Euclidean (2-norm)
